...
\item Consider a Scenario
   \begin{itemize}
   \item A scientist believes a certain rare disease does not exist at all in a population
   \item $H_0$: P = 0
...
% Cohen identifies himself as one of the statisticians who was caught making this error
% as a result many neglect the power testing their results. power is like how big your net is when catching fish
\end{tabular}
  \end{itemize}
\end{frame}
...
\begin{frame}{Replication}
  ...
  \item Typical level of power for medium effect sizes of .50 %effect size is basically the difference between H0 and Ha. If students try a new studying method and do a little bit better on an exam, low effect size. If they do way better on a test, high effect size
    \begin{itemize}
...
\begin{frame}{The Nil Hypothesis}
  ...
  \item Cohen states that the Nil Hypothesis is always false % even if different only to a tiny degree, a large enough sample will lead to its rejection
...
\begin{frame}{The Nil Hypothesis cont.}
  \begin{itemize}
  \item Even if a test is statistically significant with $p < .000001$, there can be ambient correlation noise among often arbitrarily paired variables %aka crud factor
  ...
  \item The use of a Bonferoni adjustment is then adjusting for non-existent alpha error which in turn overestimates the population effect size %using an adjustment for alpha error on a nil hypothesis are adjusting for a nonexistant alpha error
  \item Again, typically little is learned from A is larger than B $(p < .01)$ %all that is learned from failing to reject is the direction of difference between A and B is uncertain, we need amounts
  \end{itemize}
\end{frame}
...
\begin{frame}{Quantifying Effects}
  ...
  \item Scientists cannot only record pulling a rubberband makes it longer, how much longer is necessary % we need amount and direction to measure differences between A and B
  \item Correlation coefficients depend on the selection of the population, regression coefficients do not % to make raw correlation coefficients useful, scientists need to start agreeing on standardized units
  \item Correlations do not provide insight to causal strength, within group variation can change a correlations strength %researchers are stuck measuring, if you pull on it, it gets longer, and forego saying how much longer it gets
  \item Cohen's d and f | like correlations, these values are impacted by the variability across the population %cohens d and f are standardized measures of effect sizes
...
\begin{frame}{What to do?}
  ...
  \end{itemize}
  \item 3. More emphasis on confidence intervals and effect sizes % confidence intervals contain everything testing a nil hypothesis contains and all of the effect size and crud factor
  \begin{itemize}
  \item Confidence intervals aren't frequently used because they expose large variability
  \end{itemize}
  \item 4. Move away from the point "nil hypothesis" to "good-enough" range null hypotheses which will make NHST and power more useful %dont set the Ho as a new drug and placebo have no difference, but set the Ho to the new drug lowers blood pressure by no more than 5 mmHg (this may be considered not practically significant)
  \end{itemize}
\end{frame}
